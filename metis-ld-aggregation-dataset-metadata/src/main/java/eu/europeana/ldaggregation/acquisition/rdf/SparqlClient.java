...
//            ResultSetFormatter.out(System.out, results, query);
	...
	public int queryWithPaging(String queryString, int resultsPerPage, String orderVariableName, Handler handler)
		...
		while (!concluded) {
			String fullQuery = String.format("%s %s\n%s" + "LIMIT %d\n" + "OFFSET %d ", queryPrefix, queryString,
					(orderVariableName == null ? "" : "ORDER BY (" + orderVariableName + ")\n"), resultsPerPage,
					offsett[0]);
			if (debug)
				System.out.println(fullQuery);
...
//									break;
								...
								System.err.println(
										"Error on record handler: " + (resource == null ? "?" : resource.getURI()));
								e.printStackTrace();
								System.err.println("PROCEEDING TO NEXT URI");
							}
							offsett[0]++;
...
////					oLit = solution.getLiteral("o");
////				}
////				model.add(model.createStatement(subjRes, createInModel.createProperty(pRes.getURI()), oRes==null ? oLit : oRes));
//				return true;
//			}
//		});
//		return model;
