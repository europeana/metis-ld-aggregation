...
//    File sourceRdf=new File("C:\\Users\\nfrei\\Desktop\\data\\sparql-construct\\7-000spaondntfoto.2.nt");//huge dataset
    ...
    int mb = 1024 * 1024;
    DecimalFormat ROUNDED_DOUBLE_DECIMALFORMAT = new DecimalFormat("####0.00", new DecimalFormatSymbols(Locale.ENGLISH));
    ...
    System.out.println(ROUNDED_DOUBLE_DECIMALFORMAT.format((Runtime.getRuntime().totalMemory() - Runtime.getRuntime().freeMemory()) / mb ));

    Instant startQuery=Instant.now();
    
    Model edmModel = new RdfMapperBySparqlContruct(sourceModel, query).convert();
//    System.out.println(edmModel.listStatements().toList());
